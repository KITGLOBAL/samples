...
import { UseCache } from 'src/utils/http-cache.interceptor';
import { Pagination } from 'src/utils/pagination.decorator';
import { ApiModelOkResponse } from 'src/utils/swagger/ok-response.swagger';
import { PaginationDoc } from 'src/utils/swagger/pagination.swagger';
import { ApiPaginationResponse } from 'src/utils/swagger/pagination-response.swagger';
import { InjectUser } from 'src/utils/user.decorator';
import { OptionalAuth, UserGuard } from 'src/utils/user.guard';
...
export class UserController {
  ...
  @ApiQuery({
    name: 'mockip',
    type: String,
    description: 'FOR TESTING ONLY. Override your ip for this method',
    required: false,
  })
  ...
  @UseCache()
  @OptionalAuth()
  @ApiModelOkResponse(GeoByIpDto)
  async getInfoByIp(
    @Ip() originalIp: string,
    @Headers('X-Real-IP') realIp: string,
    @InjectUser() user,
    @Query('mockip') mockIp?: string,
  ): Promise<GeoByIpDto> {
    let ip: string;

    if (!ip && mockIp) ip = mockIp;
    if (!ip && realIp) ip = realIp.split(',')[0];
    if (!ip && originalIp) ip = originalIp.split(':').pop();

    console.log(ip, originalIp, realIp, mockIp);

    return this.userService.getGeoByIp(ip, user?.sub);
  }
  ...
  async getUsers(
    ...
    return this.userService.find(filter, meta);
  }
  ...
  async checkCredentials(
    @Query() query: CheckCredentialsDto,
  ): Promise<{ exists: boolean }> {
    const exists = await this.userService.checkCredentials(query);
  ...
  @ApiBearerAuth('user-token')
  @UseGuards(UserGuard)
  async getMe(@InjectUser() user): Promise<UserDocument> {
    return this.userService.findOne({ firebaseId: user.sub });
  }

  @Get('/:id')
  @ApiModelOkResponse(User)
  async getUserById(@Param('id') id: string): Promise<UserDocument> {
    return this.userService.findOne({ firebaseId: id });
  }
  ...
  async createMe(
    ...
    createUserDto.firebaseId = user.sub;
  ...
  async updateMe(
    ...
    return this.userService.update({ firebaseId: user.sub }, updateUserDto);
  }
  ...
  @HttpCode(204)
  @UseGuards(UserGuard)
  async deleteUser(@InjectUser() user): Promise<void> {
    await Promise.all([
      this.userService.delete({ firebaseId: user.sub }),
      this.firebaseService.auth().deleteUser(user.sub),
    ]);

    return;
  }

  @MessagePattern('user.questionVote')
  async onQuestionVote(@Payload() data: { user: string }) {
    console.log('user.questionVote');
  ...
  @MessagePattern('user.politicVote')
  async onPoliticVote(@Payload() data: { user: string }) {
    console.log('user.politicVote');
  ...
  async onUserConnected(
    ...
    this.userService.connected(data);
    return null;
  }
  ...
  async onUserDisconnected(
    ...
    setTimeout(() => this.userService.disconnected(data), 2000);
    return null;
  }
}
